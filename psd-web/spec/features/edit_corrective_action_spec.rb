...
RSpec.feature "Edit corrective action", :with_stubbed_elasticsearch, :with_stubbed_mailer do
  ...
  let(:new_date_received)    { corrective_action.date_decided - 1.day }
  ...
  let(:new_measure_type)      do
    (CorrectiveAction::MEASURE_TYPES - [corrective_action.measure_type]).sample == CorrectiveAction::MEASURE_TYPES[0] ? "Yes" : "No, it’s voluntary"
  end
  ...
  context "when a single product is chosen" do
    it "provides a mean to edit a corrective action" do
      ...
      fill_in "Day",            with: new_date_received.day
      fill_in "Month",          with: new_date_received.month
      fill_in "Year",           with: new_date_received.year
      select new_legislation,   from: "Under which legislation?"
      select business_two.trading_name, from: "Business"
      choose new_measure_type
      ...
      expect(page).to have_summary_item(key: "Date of action",      value: new_date_received.to_s(:govuk))
      expect(page).to have_summary_item(key: "Product",             value: new_date_received.to_s(:govuk))
      expect(page).to have_summary_item(key: "Legislation",         value: new_legislation)
      expect(page).to have_summary_item(key: "Type of action",      value: new_measure_type)
