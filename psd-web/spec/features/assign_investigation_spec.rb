...
RSpec.feature "Assigning an investigation", :with_stubbed_elasticsearch, :with_stubbed_mailer, type: :feature do
  ...
  scenario "assign case to the same owner" do
    choose user.name
    click_button "Continue"

    fill_and_submit_assign_reason_form
  ...
  scenario "assign case to other user in same team" do
    ...
    expect_activity_page_to_show_case_reassignment_to(another_active_user)
  end

  scenario "assign case to your team" do
    ...
    expect_activity_page_to_show_case_reassignment_to(user.team)
  end

  scenario "a case assigned to someone else in another team can no longer be re-assigned by original owner" do
    choose("Someone else")
    ...
    expect_activity_page_to_show_case_reassignment_to(another_active_user_another_team)

    visit "/cases/#{investigation.pretty_id}/assign/choose"
    expect(page).to have_css(".govuk-grid-row p", text: "You do not have permission to change the case owner.")
  end

  def fill_and_submit_assign_reason_form
  ...
  def expect_activity_page_to_show_case_reassignment_to(owner)
    click_link "Activity"
