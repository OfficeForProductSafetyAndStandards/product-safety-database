...
RSpec.describe AuditActivity::Investigation::UpdateCoronavirusStatus, :with_stubbed_mailer, :with_stubbed_elasticsearch do
  ...
  describe ".from" do
    it "creates an audit activity", :aggregate_failures do
      ...
      expect(audit_activity.email_update_text).to eq("#{investigation.case_type.capitalize} #{investigation.pretty_id} is not related to the coronavirus outbreak. This status was updated by #{user.decorate.display_name}.")
