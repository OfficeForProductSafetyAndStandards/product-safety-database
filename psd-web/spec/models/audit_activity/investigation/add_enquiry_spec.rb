...
RSpec.describe AuditActivity::Investigation::AddEnquiry, :with_stubbed_elasticsearch, :with_stubbed_mailer do
  ...
  describe "#build_body" do
    it "stores the body" do
      expect(activity.body).to eq("**Enquiry details**<br><br>#{enquiry.description}<br><br>Assigned to #{enquiry.assignable.display_name}.")
    end
    ...
    context "when case is coronavirus related" do
      ...
      it "adds text to the body" do
        expect(activity.body).to eq("**Enquiry details**<br><br>Case is related to the coronavirus outbreak.<br><br>#{enquiry.description}<br><br>Assigned to #{enquiry.assignable.display_name}.")
      end
    end
  end
end
