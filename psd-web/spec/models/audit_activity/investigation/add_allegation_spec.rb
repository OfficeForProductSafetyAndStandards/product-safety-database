...
RSpec.describe AuditActivity::Investigation::AddAllegation, :with_stubbed_elasticsearch, :with_stubbed_mailer do
  ...
  describe "#build_body" do
    it "stores the body" do
      expect(activity.body).to eq("**Allegation details**<br><br>Hazard type: **#{allegation.hazard_type}**<br><br>#{allegation.description}<br><br>Case owner: #{allegation.assignable.decorate.display_name}")
    end
    ...
    context "when case is coronavirus related" do
      ...
      it "adds text to the body" do
        expect(activity.body).to eq("**Allegation details**<br><br>Case is related to the coronavirus outbreak.<br>Hazard type: **#{allegation.hazard_type}**<br><br>#{allegation.description}<br><br>Case owner: #{allegation.assignable.decorate.display_name}")
      end
    end
  end
end
