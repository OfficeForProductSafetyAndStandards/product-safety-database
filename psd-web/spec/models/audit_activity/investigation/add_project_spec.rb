...
RSpec.describe AuditActivity::Investigation::AddProject, :with_stubbed_elasticsearch, :with_stubbed_mailer do
  ...
  describe "#build_body" do
    it "stores the body" do
      expect(activity.body).to eq("**Project details**<br><br>#{project.description}<br><br>Case owner: #{project.owner.display_name}")
    end
    ...
    context "when case is coronavirus related" do
      ...
      it "adds text to the body" do
        expect(activity.body).to eq("**Project details**<br><br>Case is related to the coronavirus outbreak.<br><br>#{project.description}<br><br>Case owner: #{project.owner.display_name}")
      end
    end
  end
end
