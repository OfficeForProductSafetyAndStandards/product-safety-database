...
RSpec.describe NotifyMailer, :with_stubbed_elasticsearch do
  ...
  describe "#team_added_to_case_email" do
    subject(:mail) { described_class.team_added_to_case_email(edit_access, to_email: "test@example.com") }
    ...
    let(:edit_access) { create(:edit_access, message: message, added_by_user: added_by_user, collaborator: collaborator_team) }
      ...
      context "when the user is on another team" do
        it "sets the personalisation" do
          expect(mail.govuk_notify_personalisation).to eql(
            ...
            investigation_url: investigation_url(edit_access.investigation)
          )
        end
      end
  ...
  describe "#team_deleted_from_case_email" do
    ...
    let(:edit_access) { create(:edit_access, collaborator: team_to_be_deleted) }
    let(:investigation) { edit_access.investigation }
