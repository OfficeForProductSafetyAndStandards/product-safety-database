class Investigations::TsInvestigationsController < ApplicationController
  ...
  before_action :set_skip_step, only: %i[update], if: lambda {
  ...
  before_action :set_file, only: %i[show update], if: lambda {
  ...
  before_action :store_repeat_step, only: %i[update], if: lambda {
  ...
  before_action :store_file, only: %i[update], if: lambda {
    %i[risk_assessments product_images evidence_images other_files].include? step
  }
