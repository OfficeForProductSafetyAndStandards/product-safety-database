...
RSpec.describe AuditActivity::Correspondence::AddPhoneCall, :with_stubbed_elasticsearch, :with_stubbed_mailer, :with_stubbed_antivirus do
  ...
  describe "#phone_call_by" do
    it { expect(decorated_activity.phone_call_by(viewing_user)).to eq("Phone call by #{decorated_activity.subtitle(reporting_user.name)}") }
  end
  ...
  describe "#correspondence_date" do
    it { expect(decorated_activity.correspondence_date).to eq("Date: #{correspondence.correspondence_date.to_s(:govuk)}") }
  end
