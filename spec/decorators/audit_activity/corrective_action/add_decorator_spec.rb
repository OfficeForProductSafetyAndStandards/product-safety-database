...
RSpec.describe AuditActivity::CorrectiveAction::AddDecorator, :with_stubbed_elasticsearch, :with_stubbed_mailer do
  subject(:decorated_activity) do
    described_class.decorate(
      AuditActivity::CorrectiveAction::Base.new(
        business: business,
        metadata: { corrective_action_id: corrective_action.id, updates: corrective_action.previous_changes }
      )
    )
  end
