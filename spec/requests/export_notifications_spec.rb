...
RSpec.feature "notification export", :with_opensearch, :with_stubbed_antivirus, :with_stubbed_mailer, :with_stubbed_notify, type: :feature do
  ...
  scenario "with no filters selected" do
    ...
    expect(page).to have_text investigation1.pretty_id
    expect(page).to have_text investigation2.pretty_id
    ...
    expect(spreadsheet.cell(3, 1)).to eq(investigation1.pretty_id)
    expect(spreadsheet.cell(4, 1)).to eq(investigation2.pretty_id)
    expect(spreadsheet.cell(5, 1)).to eq(enquiry_coronavirus.pretty_id)
