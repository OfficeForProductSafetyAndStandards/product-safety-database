...
RSpec.describe "Viewing a product", type: :request, with_stubbed_mailer: true, with_stubbed_opensearch: true do
  ...
  context "with timestamp", :with_errors_rendered do
    context "with invalid timestamp" do
      before { get product_version_path(product, timestamp: "invalid") }
    ...
    context "with timestamp prior to creation date" do
      before { get product_version_path(product, timestamp: 2.days.ago.to_i) }
    ...
    context "with timestamp valid for previous version" do
      before { get product_version_path(product, timestamp: creation_time.to_i) }
    ...
    context "with timestamp valid for current version" do
      before { get product_version_path(product, timestamp: Time.zone.now.to_i) }
