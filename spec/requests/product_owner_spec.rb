...
RSpec.describe "Viewing a product owner", type: :request, with_stubbed_mailer: true, with_stubbed_opensearch: true do
  ...
  context "with a product attached to a case" do
    let(:investigation) { create(:allegation, creator: user) }
    let(:investigation_product) { investigation.investigation_products.create(product:) }
    ...
    context "with an open case" do
      context "with an owner" do
        ...
        it "responds with a 200 status" do
          get owner_investigation_investigation_product_path(investigation, investigation_product)
          expect(response).to have_http_status(:ok)
        end
      end
    ...
    context "with a closed case" do
      before { ChangeCaseStatus.call! investigation:, new_status: "closed", user: }
