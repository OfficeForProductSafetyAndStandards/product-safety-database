...
RSpec.describe AddTeamToNotification, :with_stubbed_mailer, :with_stubbed_opensearch do
  ...
  context "with no user parameter" do
    let(:result) { described_class.call(team:, notification:) }
  ...
  context "with required parameters" do
    ...
    context "when the team has an email address" do
      it "notifies the team", :aggregate_failures do
        ...
        expect(email.action_name).to eq("team_added_to_notification_email")
      end
    end
    ...
    context "when the silent parameter is true", :with_test_queue_adapter do
      ...
      it "does not send any emails" do
        expect { result }.not_to have_enqueued_mail(NotifyMailer, :team_added_to_notification_email)
      end
    end
    ...
    context "when the team has already been added to the case" do
      ...
      it "does not create a new collaboration record" do
        expect { result }.not_to change(notification.collaborations, :count)
      end
      ...
      it "does not create an audit activity record" do
        expect { result }.not_to change(notification.activities, :count)
      end
    end
  end
end
