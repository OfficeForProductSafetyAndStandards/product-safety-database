...
RSpec.describe ChangeCaseCoronavirusStatus, :with_stubbed_opensearch, :with_test_queue_adapter do
  describe ".call" do
    # Create the case before test run to ensure we only check activity generated by the test
    subject(:result) { described_class.call(investigation:, status: new_status, user:) }
    ...
    context "with no investigation parameter" do
      subject(:result) { described_class.call(user:, status: new_status) }
    ...
    context "with no user parameter" do
      subject(:result) { described_class.call(investigation:, status: new_status) }
