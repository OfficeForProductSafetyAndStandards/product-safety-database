  ...
  context "with required parameters" do
    ...
    context "when the team has an email address" do
      it "notifies the team", :aggregate_failures do
        ...
        expect(email.action_name).to eq("team_removed_from_notification_email")
      end
    end
    ...
    context "when the silent parameter is true", :with_test_queue_adapter do
      ...
      it "does not send any emails" do
        expect { result }.not_to have_enqueued_mail(NotifyMailer, :team_removed_from_notification_email)
      end
    end
    ...
    it "adds an audit activity record", :aggregate_failures do
      ...
      expect(last_added_activity.decorate.title(nil)).to eql("Testing team removed from notification")
    end
  end
end
