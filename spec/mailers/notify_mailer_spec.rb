...
RSpec.describe NotifyMailer, :with_stubbed_opensearch do
  ...
  describe "#send_email_reminder" do
    ...
    # Helper method to create the mail object
    subject(:mail) do
      described_class.send_email_reminder(
        user: user,
    ...
    let(:user) { create(:user) }
    let(:pretty_id) { "2024-0002" }
    let(:title) { "Test Notification" }
    let(:remaining_days) { 3 }
    let(:days) { 87 }
    let(:last_line) { "This is the last line" }

    # Common tests for both final and non-final reminder emails
    shared_examples "common email attributes" do
      it "sets the recipient of the email" do
        expect(mail.to).to eq([user.email])
      end

      it "sets the template ID" do
        expect(mail.govuk_notify_template).to eq(described_class::TEMPLATES[:draft_notification_reminder])
      end
    ...
    # Tests for personalisation attributes
    shared_examples "personalisation attributes" do |attribute, value_method|
      it "sets the #{attribute} in personalisation attributes" do
        expect(mail.govuk_notify_personalisation).to include(attribute => send(value_method))
      end
    end

    context "when it is a final reminder email" do
      let(:last_reminder) { true }

      include_examples "common email attributes"

      # Test personalisation attributes
      it "sets the name in personalisation attributes" do
        expect(mail.govuk_notify_personalisation).to include(name: user.name)
      end

      include_examples "personalisation attributes", :remaining_days, :remaining_days
      include_examples "personalisation attributes", :days, :days
      include_examples "personalisation attributes", :title, :title
