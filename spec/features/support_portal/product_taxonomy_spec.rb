...
RSpec.feature "Product taxonomy", :with_stubbed_antivirus, :with_stubbed_mailer, :with_stubbed_notify, :with_test_queue_adapter, type: :feature do
  ...
  let(:invalid_file_blank_rows) { Rails.root.join "test/fixtures/files/taxonomy_blank_rows.xlsx" }
  ...
  scenario "Updating the product taxonomy" do
    ...
    attach_file "product_taxonomy_import[import_file]", invalid_file
    click_on "Upload product taxonomy file"

    expect(page).to have_link("The selected file has errors in the file format", href: "#product-taxonomy-import-import-file-field-error")

    attach_file "product_taxonomy_import[import_file]", invalid_file_too_many_worksheets
    click_on "Upload product taxonomy file"

    expect(page).to have_link("The selected file has too many worksheets", href: "#product-taxonomy-import-import-file-field-error")

    attach_file "product_taxonomy_import[import_file]", invalid_file_too_many_columns
    click_on "Upload product taxonomy file"

    expect(page).to have_link("The selected file has extra data on row 2", href: "#product-taxonomy-import-import-file-field-error")

    attach_file "product_taxonomy_import[import_file]", invalid_file_blank_rows
    click_on "Upload product taxonomy file"

    expect(page).to have_link("The selected file has incomplete data on row 2", href: "#product-taxonomy-import-import-file-field-error")
