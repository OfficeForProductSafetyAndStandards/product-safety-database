...
RSpec.feature "Add an attachment to a notification", :with_stubbed_antivirus, :with_stubbed_mailer, type: :feature do
  ...
  scenario "Adding an attachment that is not an image" do
    ...
    expect_to_be_on_add_attachment_to_a_notification_page
    ...
    change_attachment_to_have_simulate_virus(notification.reload)
  ...
  context "when image is too small" do
    it "shows error" do
      ...
      expect_to_be_on_add_image_to_a_notification_page

      attach_file "image_upload[file_upload]", empty_image

      click_button "Upload"

      expect(page).to have_current_path("/cases/#{notification.pretty_id}/image_uploads")
  ...
  def change_attachment_to_have_simulate_virus(notification)
    blob = notification.documents.first.blob
