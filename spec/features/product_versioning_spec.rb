...
RSpec.feature "Product versioning", :with_opensearch, :with_stubbed_mailer, type: :feature do
  ...
  scenario "editing a product" do
    ...
    expect(page).to have_selector("dd.govuk-summary-list__value", text: "psd-#{product.id}_#{investigation.date_closed.to_i} - The PSD reference number for this version of the product record - as recorded when the case was closed: #{investigation.date_closed.to_formatted_s(:govuk)}.")
