...
RSpec.feature "Investigation listing", :with_elasticsearch, :with_stubbed_mailer, type: :feature do
  ...
  scenario "lists cases correctly sorted" do
    ...
    expect(page.find("select#sort_by option[value='#{SearchParams::RELEVANT}']")).to be_selected
    ...
    expect(page.find("select#sort_by option[value='#{SearchParams::RECENT}']")).to be_selected
    ...
    expect(page.find("select#sort_by option[value='#{SearchParams::OLDEST}']")).to be_selected
