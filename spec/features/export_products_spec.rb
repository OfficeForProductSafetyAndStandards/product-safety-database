  ...
  describe "#index as XLSX" do
    ...
    context "when logged in as a user with the psd_admin role" do
      ...
      it "exports correct sheets" do
        ...
        expect(exported_data.sheets).to eq ["product_info", "test_results", "risk_assessments", "corrective_actions"]
      end
      ...
      context "product_info sheet" do
        ...
        it "exports case_ids" do
          ...
          expect(sheet.cell(2,7)).to eq product.investigations.map(&:id).to_s
          expect(sheet.cell(3,7)).to eq other_product.investigations.map(&:id).to_s
        end
