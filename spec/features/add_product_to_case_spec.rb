...
RSpec.feature "Adding a product to a case", :with_stubbed_mailer, :with_stubbed_opensearch do
  ...
  scenario "Finding and linking an existing product" do
    ...
    expect(page).to have_css(".govuk-summary-list__value", text: "#{right_product.psd_ref} - The PSD reference for this product record")
    expect(page).to have_css(".govuk-summary-list__value", text:  "#{right_product.psd_ref}_#{investigation.reload.investigation_products.first.investigation_closed_at.to_i} - The PSD reference for this product record" )
