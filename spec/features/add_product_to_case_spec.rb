...
RSpec.feature "Adding a product to a case", :with_stubbed_mailer, :with_stubbed_opensearch do
  ...
  scenario "Finding and linking an existing product" do
    ...
    expect(page).to have_css(".govuk-summary-list__value", text: "#{right_product.psd_ref} - The PSD reference number for this version of the product record", exact: true)
