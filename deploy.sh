...
# Deploy the app
cf7 push $APP_NAME -f $MANIFEST_FILE --app-start-timeout 180 --var app-name=$APP_NAME --var psd-host=$DOMAIN --var web-disk-quota=$WEB_DISK_QUOTA --var worker-disk-quota=$WORKER_DISK_QUOTA --var web-memory=$WEB_MEMORY --var worker-memory=$WORKER_MEMORY --var web-instances=$WEB_INSTANCES --var worker-instances=$WORKER_INSTANCES --var web-max-threads=$WEB_MAX_THREADS --var worker-max-threads=$WORKER_MAX_THREADS --strategy rolling

# follow up PR to remove
cf run-task psd-web -c "export \$(./env/get-env-from-vcap.sh) && rails db:migration:business_removed_audit db:migration:business_added_audit"
