...
# This model represents a "product taxonomy import" that is initiated from the Support Portal by uploading a product taxonomy file.
# This file is attached as the `import_file`. The `ProductTaxonomyImportJob` background job then runs to process this file,
# update the `ProductCategory` and `ProductSubcategory` models, generate an export Excel file with the processed taxonomy for reference,
# and create a new bulk upload template Excel file for uploading products with data validation based on the newly-updated product
...
private
  ...
  def file_is_an_excel_workbook
    ...
    errors.add(:import_file, :incorrect_type, message: "The selected file must be an Excel file (XLSX)")
  end
  ...
  def file_is_in_expected_format
    ...
    import_file.open do |file|
      begin
        ...
        errors.add(:import_file, :invalid_format, message: "The selected file has errors in the file format")
      ...
      if workbook.worksheets[0].sheet_data.rows.empty? || workbook.worksheets[0].sheet_data.rows.length == 1
  ...
  def import_successful?
    ProductCategory.where("created_at > ?", created_at).exists? &&
      ProductSubcategory.where("created_at > ?", created_at).exists?
  end
end
