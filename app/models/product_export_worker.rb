  ...
  def export(products, product_export_id)
    ...
    Axlsx::Package.new do |p|
      ...
      book.add_worksheet name: "product_info" do |sheet_investigations| # rubocop:disable Metrics/BlockLength
        sheet_investigations.add_row %w[LOLNOTID affected_units_status authenticity barcode batch_number brand case_ids category country_of_origin
      ...
      p.serialize(Rails.root.join("ProductExport#{product_export_id}.xlsx"))

      product_export.export_file.attach(io: File.open(Rails.root.join("bbbb.xlsx")), filename: "test.xlsx")
    end
  end
end
