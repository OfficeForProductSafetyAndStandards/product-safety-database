class SecondaryAuthentication
  ...
  def otp_expired?
    user.direct_otp_sent_at && (user.direct_otp_sent_at + OTP_EXPIRY_SECONDS) < Time.zone.now.utc
  end

  def otp_locked?
    user.second_factor_attempts_locked_at.present? && (user.second_factor_attempts_locked_at + MAX_ATTEMPTS_COOLDOWN.seconds) > Time.zone.now.utc
  end
  ...
  def try_to_unlock_secondary_authentication
    if user.second_factor_attempts_locked_at && (user.second_factor_attempts_locked_at + MAX_ATTEMPTS_COOLDOWN.seconds) < Time.zone.now.utc
...
private
  ...
  def increment_attempts_and_try_to_lock
    ...
    if user.second_factor_attempts_count > MAX_ATTEMPTS
      user.update!(second_factor_attempts_locked_at: Time.zone.now.utc, second_factor_attempts_count: 0)
    end
  end
