class CorrectiveAction < ApplicationRecord
  ...
  validates :summary, inclusion: { in: Rails.application.config.corrective_action_constants["actions"] }, unless: -> { summary == "other" }
  validates :other_action, presence: true, if: -> { summary == "other" }
