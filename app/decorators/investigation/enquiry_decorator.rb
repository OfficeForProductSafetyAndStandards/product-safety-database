class Investigation < ApplicationRecord
  ...
  class EnquiryDecorator < InvestigationDecorator
    def source_details_summary_list(view_protected_details: false)
      ...
      rows = []
      rows << { key: { text: "Received date" }, value: { text: date_received.to_formatted_s(:govuk) } } if should_display_date_received?
      rows << { key: { text: "Received by" }, value: { text: received_type.upcase_first } } if should_display_received_by?
      rows << { key: { text: "Source type" }, value: { text: complainant.complainant_type } }
      rows << { key: { text: "Contact details" }, value: { text: contact_details } }
