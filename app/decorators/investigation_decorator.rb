class InvestigationDecorator < ApplicationDecorator
  ...
  def contact_details_list
    h.tag.ul(class: "govuk-list govuk-list--bullet govuk-list--spaced") do
      lis = []
      lis << h.tag.li(complainant.name) unless complainant.name.blank?
      lis << h.tag.li("Telephone: #{complainant.phone_number}") unless complainant.phone_number.blank?
      lis << "<li>Email: #{helpers.mail_to(complainant.email_address)}</li>"
      lis << h.tag.li(complainant.other_details) unless complainant.other_details.blank?
      lis.join.html_safe
    end
  end
